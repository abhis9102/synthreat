...
meaningful, so it's safe to treat as an inert string.* A device token, a registration ID, a numeric-
looking parameter — these get treated as opaque values with no structure worth validating, which
...
- Endpoints that share a query-building helper function with several sibling routes. If one endpoint
  using that helper is vulnerable, the others usually are too — and just as often, one sibling turns
...
condition, one to always-false — produces two consistently different response shapes. That's the real
confirmation: the application's actual query logic is being altered, not just its error output.
...
- **Server-side request forgery** — a structurally different class, but frequently found on the same
  target, and often traceable to the same root cause: trusting an input because of where it's used
