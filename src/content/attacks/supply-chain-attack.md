...
without independently re-verifying it.* That assumption has to exist — no organization can realistically
audit every line of every dependency it runs — but it also means the moment the vendor's own build
process, update mechanism, or account credentials are compromised, that trust is exactly what carries
the attacker's code straight through the front door. What breaks isn't a specific technical control;
it's the entire chain-of-trust model that software distribution is built on, at whichever link in that
chain turns out to be weakest.
